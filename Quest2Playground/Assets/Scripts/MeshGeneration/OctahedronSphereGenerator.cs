...
public class OctahedronSphereGenerator
{
    ...
    public static Mesh Create(int subdivisions, float radius)
    {
        ...
        {
            subdivisions = 0;
            Debug.LogWarning("Octahedron Sphere subdivisions increased to minumum, wich is 0.");
        }
    ...
    private static void CreateOctahedron(Vector3[] vertices, int[] triangles, int resolution)
    {
        ...
        for(int i = 1; i <= resolution; i++)
        {
            ...
            to = Vector3.Lerp(Vector3.down, Vector3.forward, progress);
            vertices[v++] = to;
        ...
        for (int i = resolution - 1; i >= 1; i--)
        {
            ...
            vertices[v] = Vector3.Lerp(Vector3.up, Vector3.forward, progress);
            to = vertices[v++];
    ...
    private static void CreateTangents(Vector3[] vertices, Vector4[] tangents)
    {
        ...
        tangents[0] = new Vector3(-1f, 0, -1f).normalized;
        tangents[vertices.Length - 4] = tangents[0];

        tangents[1] = new Vector3(1f, 0, -1f).normalized;
        tangents[vertices.Length - 3] = tangents[1];

        tangents[2] = new Vector3(1f, 0, 1f).normalized;
        tangents[vertices.Length - 2] = tangents[2];

        tangents[3] = new Vector3(-1f, 0, 1f).normalized;
    ...
    private static void CreateUV(Vector3[] vertices, Vector2[] uv)
    {
        ...
        uv[0].x = 0.125f;
        uv[vertices.Length - 4].x = 0.125f;

        uv[1].x = 0.375f;
        uv[vertices.Length - 3].x = 0.375f;

        uv[2].x = 0.625f;
        uv[vertices.Length - 2].x = 0.625f;

        uv[3].x = 0.875f;
        uv[vertices.Length - 1].x = 0.875f;
    }
}
