...
public class SimplexNoiseGenerator {
    ...
    public float coherentNoise(float x, float y, float z, SimplexNoiseSettings noiseSettings)
    {
        Vector3 v3 = new Vector3(x, y, z) / noiseSettings.multiplier;
        float val = 0;
        float amplitude = noiseSettings.amplitude;
        for (int n = 0; n < noiseSettings.octaves; n++)
        {
            val += noise(v3.x, v3.y, v3.z) * amplitude;
            v3 *= noiseSettings.lacunarity;
